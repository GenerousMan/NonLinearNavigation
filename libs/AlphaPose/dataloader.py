...
from libs.AlphaPose.SPPE.src.utils.img import load_image, cropBox, im_to_torch
from libs.AlphaPose.opt import opt
from libs.AlphaPose.yolo.preprocess import prep_image, prep_frame, inp_to_image
from libs.AlphaPose.pPose_nms import pose_nms, write_json
from libs.AlphaPose.SPPE.src.utils.eval import getPrediction
from libs.AlphaPose.yolo.util import write_results, dynamic_write_results
from libs.AlphaPose.yolo.darknet import Darknet
...
if opt.vis_fast:
    from libs.AlphaPose.fn import vis_frame_fast as vis_frame
else:
    from libs.AlphaPose.fn import vis_frame
...
class CustomLoader:
    ...
    def __init__(self, videos, batchSize=1, queueSize=50):
        self.videos = videos
        self.stopped = False
    ...
    def update(self):
        ...
        # image_index = 0
        # temp_folder = './edit/images_temp/'
        # file_folder = os.path.join(temp_folder, str(uuid.uuid1()))
        # os.makedirs(file_folder)

        frame_interval = self.videos[video_index].fps * self.videos[video_index].sample_time_interval
        stream = cv2.VideoCapture(self.videos[video_index].path)
        assert stream.isOpened(), 'Cannot capture source'
        ...
        for i in range(self.num_batches):
            ...
            for k in range(i * self.batchSize, min((i + 1) * self.batchSize, self.datalen)):
                inp_dim = int(opt.inp_dim)
                target_frame_number = int(round(next_frame_number))
                while frame_number < target_frame_number:
                    (grabbed, frame) = stream.read()
                    frame_number += 1
                    continue
                (grabbed, frame) = stream.read()
                ...
                if not grabbed:
                    # 一个视频读取结束
                    if video_index + 1 == len(self.videos):
                        # 所有都读取结束了
                        self.Q.put((None, None, None, None))
                        # print('===========================> All videos load finish: {}fs'.format())
                        sys.stdout.flush()
                        return
                    else:
                        # 继续读取下一个视频
                ...
                # image_path = os.path.join(file_folder, '{}.jpg'.format(image_index))
                # image_index += 1
...
class DetectionLoader:
    def __init__(self, dataloder, batchSize=1, queueSize=1024):
        # initialize the file video stream along with the boolean
        # used to indicate if the thread should be stopped or not
        self.det_model = Darknet("libs/AlphaPose/yolo/cfg/yolov3-spp.cfg")
        self.det_model.load_weights('libs/AlphaPose/models/yolo/yolov3-spp.weights')
...
class VideoDetectionLoader:
    def __init__(self, path, batchSize=4, queueSize=256):
        # initialize the file video stream along with the boolean
        # used to indicate if the thread should be stopped or not
        self.det_model = Darknet("lib/AlphaPose/yolo/cfg/yolov3-spp.cfg")
        self.det_model.load_weights('lib/AlphaPose/models/yolo/yolov3-spp.weights')
        self.det_model.net_info['height'] = opt.inp_dim
        self.det_inp_dim = int(self.det_model.net_info['height'])
        assert self.det_inp_dim % 32 == 0
        assert self.det_inp_dim > 32
                ...
                if boxes is None:
                    if opt.save_img or opt.save_video or opt.vis:
                        ...
                        if opt.vis:
                            ...
                            cv2.waitKey(30)
                        if opt.save_img:
                            cv2.imwrite(os.path.join(opt.outputpath, im_name), img)
                        if opt.save_video:
